...
class ExcelFormatter(BaseFormatter):
    ...
    def export(
        ...
        if self.config.layout == OutputLayout.NORMALIZED:
            return self._export_normalized(installations, facilities, systems, work_orders, metadata)
        else:
            return self._export_denormalized(installations, facilities, systems, work_orders, metadata)
    ...
    def _export_normalized(
        ...
        tables = self.transformer.create_normalized_tables(installations, facilities, systems, work_orders)
        ...
        with pd.ExcelWriter(self.config.file_path, engine="openpyxl") as writer:
            for table_name, df in tables.items():
                if df is not None and not df.empty:
                    ...
                    sheet_name = sheet_name_map.get(table_name, table_name.replace("_", " ").title())
            ...
            if metadata:
                ...
                if not meta_df.empty:
                    meta_df.to_excel(writer, sheet_name=output.excel_sheet_metadata, index=False)
    ...
    def _export_denormalized(
        ...
        rows = self.transformer.create_denormalized_rows(installations, facilities, systems, work_orders)
