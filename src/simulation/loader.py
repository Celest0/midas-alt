...
from ..enums import UFCGrade, WO_Priority, WO_Status, WO_TradeSkill
...
class SimulationDataLoader:
    ...
    def load(self, dataset_path: str | Path) -> GenerationResult:
        ...
        return self._build_generation_result(tables)
    ...
    def _build_generation_result(self, tables: dict[str, pd.DataFrame]) -> GenerationResult:
    ...
    def _build_facilities(self, table: pd.DataFrame) -> list[Facility]:
        ...
        for row in table.to_dict(orient="records"):
            ...
            facilities.append(
                Facility(
                    id=_required_text(row, "id"),
                    facility_type_key=_int_value(row, "facility_type_key"),
